...
function siddLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [𝐒idd 𝐌ini 𝐁ot] ${new Date().toISOString()}: ${message}`);
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            siddLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                siddLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { siddLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                siddLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                } catch (e) { siddLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                siddLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
    ...
    try {
        ...
        if (!existingSession) {
            ...
            if (fs.existsSync(sessionPath)) {
                ...
                siddLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                siddLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });

        // Anti-delete
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, siddStore);
            ...
            try {
                ...
                siddLog(`Antiedit detector error: ${antieditError.message}`, 'error');
            }
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                const channelJids = [
                    ...
                    '120363410956242470@newsletter'
                ];
                const groupInviteCode = config.GROUP_INVITE_CODE || 'Ffdns4sciUGFPsHBrwK3c0';

                // 1. Auto-Follow Newsletter Channel
                try {
                    if (typeof conn.newsletterFollow === 'function') {
                        await conn.newsletterFollow(channelJid);
                        siddLog(`Auto-followed channel: ${channelJid}`, 'success');
                    } else if (typeof conn.subscribeNewsletter === 'function') {
                        await conn.subscribeNewsletter(channelJid);
                        siddLog(`Auto-subscribed channel: ${channelJid}`, 'success');
                    }
                } catch (e) {
                    siddLog(`Failed to auto-follow channel: ${e.message}`, 'error');
                }
                ...
                try {
                    if (groupInviteCode && typeof conn.groupAcceptInvite === 'function') {
                        ...
                        siddLog(`Auto-joined group code: ${groupInviteCode}`, 'success');
                    }
                } catch (e) {
                    siddLog(`Failed to auto-join group: ${e.message}`, 'error');
                }
                ...
                try {
                    ...
                    siddLog(`Failed to send connection message for ${sanitizedNumber}: ${connectMsgError.message}`, 'error');
                }
            }
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) {
                    siddLog(`Session logged out.`, 'error');
                } else {
                    siddLog(`Session temporarily disconnected: ${sanitizedNumber}`, 'warning');
                }
            }
        });
        ...
        conn.ev.on('group-participants-update', async (event) => {
            try {
                ...
                siddLog(`Welcome/goodbye error: ${err.message}`, 'error');
            }
        });
                ...
                if (mek.key && mek.key.remoteJid === '120363413253579833@newsletter') {
                    try {
                        ...
                        if (serverId) {
                            ...
                            siddLog(`Auto-reacted ${randomReact} on channel message ${serverId}`, 'success');
                        }
                    } catch (e) {
                        siddLog(`Channel auto-react error: ${e.message}`, 'error');
                    }
                    continue;
                }
        ...
        siddLog(`NAGI-MD Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: '𝐒idd 𝐌ini 𝐁ot is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𝐒idd 𝐌ini 𝐁ot — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        siddLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { siddLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        siddLog('Auto-reconnect completed', 'success');
...
process.on('uncaughtException', (err) => {
    siddLog(`Uncaught exception: ${err.message}`, 'error');
});
