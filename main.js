    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // Auto-follow ONLY these newsletter channels.
                // The second channel is intentionally added here only; the
                // auto-react handler below remains limited to the first channel.
                const channelJids = [
                    ...
                    if (typeof conn.newsletterFollow === 'function') {
                        ...
                        for (const channelJid of channelJids) {
                            try {
                                ...
                                arslanLog(`Failed to auto-subscribe channel ${channelJid}: ${e.message}`, 'error');
                            }
                        }
                    } else {
                        arslanLog('No newsletter follow method available in this Baileys build.', 'warning');
                    }
                } catch (e) {
                    arslanLog(`Newsletter auto-follow error: ${e.message}`, 'error');
                }
                ...
                try {
                    if (groupInviteCode && typeof conn.groupAcceptInvite === 'function') {
                        ...
                        arslanLog('groupAcceptInvite is not available in this Baileys build.', 'warning');
                    }
                } catch (e) {
                ...
                if (!existingSession) {
                    try {
                        ...
                        arslanLog(`Failed to send connection message for ${sanitizedNumber}: ${e.message}`, 'error');
                    }
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            ...
            for (const mek of msg.messages) {
              try {
                ...
                // Same placement/structure as main.js: handle newsletter events
                // before requiring mek.message to exist.
                if (mek.key && mek.key.remoteJid === '120363413253579833@newsletter') {
                    try {
                        const autoReactEmojis = ['❤️', '🌟', '⏳', '💘', '🪐', '💫', '🔥', '👑'];
                        const serverId = mek.key.server_id;

                        if (serverId && typeof conn.newsletterReactMessage === 'function') {
